...
public class JCVideoPlayer extends FrameLayout implements View.OnClickListener, SeekBar.OnSeekBarChangeListener, SurfaceHolder.Callback, View.OnTouchListener {
    ...
    private TasksCompletedView mProsenTv;
    ...
    private OnVideoLiser mOnVideoLiser;

    public OnVideoLiser getmOnVideoLiser() {
        return mOnVideoLiser;
    }

    public void setmOnVideoLiser(OnVideoLiser mOnVideoLiser) {
        this.mOnVideoLiser = mOnVideoLiser;
    }

    public interface OnVideoLiser{
        void onStart();
        void onEnd();
    }
    ...
    private void init(Context context) {
        ...
        mProsenTv.setOnClickListener(new OnClickListener() {
            @Override
...
//        surfaceView.setBackgroundColor(R.color.black_a10_color);
    ...
    public void setSThumb(String path){
        thumb = path;
        ImageLoader.getInstance().displayImage("http://122.114.98.206:8080/Resources/"+thumb, ivThumb, Utils.getDefaultDisplayImageOption());
    }
    public void setProgressTv(int str){
        mProsenTv.setProgress(str);
    }

    public void isDownEnd(){
        mProsenTv.setVisibility(GONE);
    }
    public void isDown(){
        mProsenTv.setVisibility(VISIBLE);
...
//            surfaceView.setBackgroundColor(R.color.black_a10_color);
    ...
    public void onClick(View v) {
        int i = v.getId();
        if (i == R.id.start ||i == R.id.start1 || i == R.id.thumb) {
            play(i);
    ...
    private void updateStartImage() {
        if (CURRENT_STATE == CURRENT_STATE_PLAYING) {
            ivStart.setImageResource(R.drawable.click_video_pause_selector);
            ivStart1.setVisibility(GONE);
        } else {
            ivStart.setImageResource(R.drawable.click_video_play_selector);
            ivStart1.setImageResource(R.drawable.icon_ueueueue);
            ivStart1.setVisibility(VISIBLE);
        }
    }
...
//        cancelDismissControlViewTimer();
    ...
    private void sendPointEvent(int type) {
        ...
        if(mOnVideoLiser!=null){
            if(type == VideoEvents.POINT_AUTO_COMPLETE){
                mOnVideoLiser.onEnd();
            }

            if(type == VideoEvents.POINT_START_ICON){
                mOnVideoLiser.onStart();
            }
        }

...
public class UpdataNewTool {
    ...
    public static UpdataNewTool getTool(ArrayList<ImageBean> paths){
            return new UpdataNewTool(paths);
    }

    public UpdataNewTool(ArrayList<ImageBean> paths){
    ...
    public void updata(){
        if(mUpIndex < mPaths.size()){
            ...
            if(mPaths.get(mUpIndex).getPath().contains("mp4") || mPaths.get(mUpIndex).getPath().contains("3gp")) {
                ...
                temp =result.getResultData().getFilePath();
                result = new Gson().fromJson(Upload.uploadFile(mPaths.get(mUpIndex).getImage()), UpdataResult.class);
                temp += ";"+result.getResultData().getFilePath();
            }else{

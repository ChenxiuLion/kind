...
public class NewDetailActivity extends KingActivity {
    ...
    public void getData(){
        HttpTool.getInstance().getUserName(getUser().getID()+"",getBaby().getID()+"", new KingCallback<GetUserName>(GetUserName.class) {
            ...
            public void onSucceed(GetUserName data) {
                HttpTool.getInstance().doUserLook(mBean, data.getResultData(getUser().getID()).getParentID(),getBaby().getID(),data.getResultData().getRelation(), new KingCallback<NewsUser>(NewsUser.class) {
                    @Override
        ...
        if(mBean.getNoticeType() == 6){
            ((TextView)findViewById(R.id.top_title)).setText("班级新闻详情");
        }else{
    ...
    public void getUsers(){
        ...
        HttpTool.getInstance().getUserLookList(mBean.getID()+"",getBaby().getID()+"", new KingCallback<UserLookList>(UserLookList.class) {
            @Override
    ...
    private BaseAdapter mUserAdapter = new BaseAdapter() {
        @Override
        public int getCount() {
